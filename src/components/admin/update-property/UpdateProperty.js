...
export default function  UpdateProperty() {
    ...
    const [developer, setdeveloper] = useState(result.developer||"")
    ...
    const [featured, setfeatured] = useState(result.featured||false)
    const [priority, setpriority] = useState(result.priority||0)

    const [two, settwo] = useState(result.two||false)
    const [three, setthree] = useState(result.three||false)
    const [four, setfour] = useState(result.four||false)
    const [five, setfive] = useState(result.five||false)
    const [plot, setplot] = useState(result.plot||false)
    const [villa, setvilla] = useState(result.villa||false)
    
    const [twoarea, settwoarea] = useState(result.twoArea||"")
    const [threearea, setthreearea] = useState(result.threeArea||"")
    const [fourarea, setfourarea] = useState(result.fourArea||"")
    const [fivearea, setfivearea] = useState(result.fiveArea||"")
    const [plotarea, setplotarea] = useState(result.plotArea||"")
    const [villaarea, setvillaarea] = useState(result.villaArea||"")

    const [twoimage, settwoimage] = useState(result.twoImage||"")
    const [threeimage, setthreeimage] = useState(result.threeImage||"")
    const [fourimage, setfourimage] = useState(result.fourImage||"")
    const [fiveimage, setfiveimage] = useState(result.fiveImage||"")
    const [plotimage, setplotimage] = useState(result.plotImage||"")
    const [villaimage, setvillaimage] = useState(result.villaImage||"")

    const [cci, setcci] = useState(result.cci||"")

    const [siteplan, setsiteplan] = useState(result.siteplanImage||"")

    const [price, setPrice] = useState(result.price||"")

    const [sp, setsp] = useState(result.sp||false)
    const [jt, setjt] = useState(result.jtfalse)
    const [gym, setgym] = useState(result.gym||false)
    const [security, setsecurity] = useState(result.security||false)
    const [cp, setcp] = useState(result.cp||false)
    const [restaurant, setrestaurant] = useState(result.restaurant||false)
    const [club, setclub] = useState(result.club||false)
    const [bc, setbc] = useState(result.bc||false)
    const [ka, setka] = useState(result.ka||false)
    const [ff, setff] = useState(result.ff||false)
    const [sm, setsm] = useState(result.sm||false)
    const [multiplexes, setmultiplexes] = useState(result.multiplexes||false)
    const [os, setos] = useState(result.os||false)
    const [fc, setfc] = useState(result.fc||false)

    const [projectArea, setprojectArea] = useState(result.projectArea||"")
    const [apartments, setapartments] = useState(result.apartments||"")
    ...
    useEffect(async() => {
        ...
        if(id)
        {
            await axios.get('/api/admin/property/'+id)
            .then(res => {
                setResult(res.data)
            })
            .catch(err =>{  
    ...
    useEffect(() => {
        ...
        setfive(result.five)
        setplot(result.plot)
        setvilla(result.villa)

        settwoarea(result.twoArea)
        setthreearea(result.threeArea)
        setfourarea(result.fourArea)
        setfivearea(result.fiveArea)
        setplotarea(result.plotArea)
        setvillaarea(result.villaArea)

        settwoimage(result.twoImage)
        setthreeimage(result.threeImage)
        setfourimage(result.fourImage)
        setfiveimage(result.fiveImage)
        setplotimage(result.plotImage)
        setvillaimage(result.villaImage)

        setcci(result.commercialComplexImage)

        setsiteplan(result.siteplanImage)
        ...
        setff(result.ff)
        setsm(result.sm)
        setmultiplexes(result.multiplexes)
        setos(result.os)
        setfc(result.fc)
        setprojectArea(result.projectArea)
    ...
    const handlesubmit=async()=>{
        ...
        var i;
        var imagesArray=result.imagesArray;
        if(image)
        {
        ...
        if(imagesArray.length>0)
        {
            ...
            await axios.post('/api/admin/property/update', mongodata)
                .then(res => {
                    toast({
            ...
            <main className="mainContent bg-gray-500">
                <div className="p-4 flex justify-center items-center w-full h-full">
                    <div className=" bg-white rounded-xl w-full md:w-8/12 lg:w-6/12 p-12">
                        <Stack spacing={3}>                            
                        <h3 className="text-3xl p-6 pb-2 text-center">Updating Property</h3>  
